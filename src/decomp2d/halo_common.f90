...
!    out = -1.0_mytype ! fill the halo for debugging
    ...
    if (s1==decomp%xsz(1)) then
       ...
       call MPI_IRECV(out(xs,ys,zs), 1, halo12, &
            neighbour(1,4), tag_s, MPI_COMM_CART_X, requests(1), ierror)
       ! receive from north
       call MPI_IRECV(out(xs,ye-level+1,zs), 1, halo12, &
            neighbour(1,3), tag_n, MPI_COMM_CART_X, requests(2), ierror)
       ! send to south
       call MPI_ISSEND(out(xs,ys+level,zs), 1, halo12, &
            neighbour(1,4), tag_s, MPI_COMM_CART_X, requests(3), ierror)
       ! send to north
       call MPI_ISSEND(out(xs,ye-level-level+1,zs), 1, halo12, &
            neighbour(1,3), tag_n, MPI_COMM_CART_X, requests(4), ierror)
       ...
       call MPI_IRECV(out(xs,ys,zs), icount, data_type, &
            neighbour(1,6), tag_b, MPI_COMM_CART_X, requests(1), ierror)
       ! receive from top
       call MPI_IRECV(out(xs,ys,ze-level+1), icount, data_type, &
            neighbour(1,5), tag_t, MPI_COMM_CART_X, requests(2), ierror)
       ! send to bottom
       call MPI_ISSEND(out(xs,ys,zs+level), icount, data_type, &
            neighbour(1,6), tag_b, MPI_COMM_CART_X, requests(3), ierror)
       ! send to top
       call MPI_ISSEND(out(xs,ys,ze-level-level+1), icount, data_type, &
            neighbour(1,5), tag_t, MPI_COMM_CART_X, requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)
#ifdef HALO_DEBUG       
       if (nrank==4) then
    ...
    ! Y-pencil   
       ...
       call MPI_IRECV(out(xs,ys,zs), 1, halo21, &
            neighbour(2,2), tag_w, MPI_COMM_CART_Y, requests(1), ierror)
       ! receive from east
       call MPI_IRECV(out(xe-level+1,ys,zs), 1, halo21, &
            neighbour(2,1), tag_e, MPI_COMM_CART_Y, requests(2), ierror)
       ! send to west
       call MPI_ISSEND(out(xs+level,ys,zs), 1, halo21, &
            neighbour(2,2), tag_w, MPI_COMM_CART_Y, requests(3), ierror)
       ! send to east
       call MPI_ISSEND(out(xe-level-level+1,ys,zs), 1, halo21, &
            neighbour(2,1), tag_e, MPI_COMM_CART_Y, requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)
       ...
       ! receive from bottom
       call MPI_IRECV(out(xs,ys,zs), icount, data_type, &
            neighbour(2,6), tag_b, MPI_COMM_CART_Y, requests(1), ierror)
       ! receive from top
       call MPI_IRECV(out(xs,ys,ze-level+1), icount, data_type, &
            neighbour(2,5), tag_t, MPI_COMM_CART_Y, requests(2), ierror)
       ! send to bottom
       call MPI_ISSEND(out(xs,ys,zs+level), icount, data_type, &
            neighbour(2,6), tag_b, MPI_COMM_CART_Y, requests(3), ierror)
       ! send to top
       call MPI_ISSEND(out(xs,ys,ze-level-level+1), icount, data_type, &
            neighbour(2,5), tag_t, MPI_COMM_CART_Y, requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)
#ifdef HALO_DEBUG
       if (nrank==4) then
          write(*,*) 'After exchange in Z'
    ...
    ! Z-pencil
       ...
       call MPI_IRECV(out(xs,ys,zs), 1, halo31, &
            neighbour(3,2), tag_w, MPI_COMM_CART_Z, requests(1), ierror)
       ! receive from east
       call MPI_IRECV(out(xe-level+1,ys,zs), 1, halo31, &
            neighbour(3,1), tag_e, MPI_COMM_CART_Z, requests(2), ierror)
       ! send to west
       call MPI_ISSEND(out(xs+level,ys,zs), 1, halo31, &
            neighbour(3,2), tag_w, MPI_COMM_CART_Z, requests(3), ierror)
       ! send to east
       call MPI_ISSEND(out(xe-level-level+1,ys,zs), 1, halo31, &
            neighbour(3,1), tag_e, MPI_COMM_CART_Z, requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)
       ...
       call MPI_IRECV(out(xs,ys,zs), 1, halo32, &
            neighbour(3,4), tag_s, MPI_COMM_CART_Z, requests(1), ierror)
       ! receive from north
       call MPI_IRECV(out(xs,ye-level+1,zs), 1, halo32, &
            neighbour(3,3), tag_n, MPI_COMM_CART_Z, requests(2), ierror)
       ! send to south
       call MPI_ISSEND(out(xs,ys+level,zs), 1, halo32, &
            neighbour(3,4), tag_s, MPI_COMM_CART_Z, requests(3), ierror)
       ! send to north
       call MPI_ISSEND(out(xs,ye-level-level+1,zs), 1, halo32, &
            neighbour(3,3), tag_n, MPI_COMM_CART_Z, requests(4), ierror)
       call MPI_WAITALL(4, requests, status, ierror)
