...
module decomp_2d
  ...
  integer, save :: MPI_COMM_CART_X, MPI_COMM_CART_Y, MPI_COMM_CART_Z 
...
contains
  ...
  subroutine decomp_2d_init(nx,ny,nz,p_row,p_col,periodic_bc)
    ...
    integer :: errorcode, ierror, row, col, iol
    ...
    ! need to create multiple topology objects: MPI_COMM_CART_X/Y/Z,
    ...
    call MPI_CART_CREATE(MPI_COMM_WORLD,2,dims,periodic, &
         ...
         MPI_COMM_CART_X, ierror)
    periodic(1) = periodic_x
    periodic(2) = periodic_z
    call MPI_CART_CREATE(MPI_COMM_WORLD,2,dims,periodic, &
         .false., MPI_COMM_CART_Y, ierror)
    periodic(1) = periodic_x
    periodic(2) = periodic_y
    call MPI_CART_CREATE(MPI_COMM_WORLD,2,dims,periodic, &
         .false., MPI_COMM_CART_Z, ierror)

    call MPI_CART_COORDS(MPI_COMM_CART_X,nrank,2,coord,ierror)
    
    ! derive communicators defining sub-groups for ALLTOALL(V)
    call MPI_CART_SUB(MPI_COMM_CART_X,(/.true.,.false./), &
         MPI_COMM_COL,ierror)
    call MPI_CART_SUB(MPI_COMM_CART_X,(/.false.,.true./), &
         MPI_COMM_ROW,ierror)
...
99  format('log',I2.2)
  ...
  subroutine decomp_info_init(nx,ny,nz,decomp)
    ...
    ! defined the buffers globally for performance reason
    ! *** TODO: consider how to save memory
    ...
    if (buf_size > decomp_buf_size) then
       ...
       if (status /= 0) then
          errorcode = 2
          call decomp_2d_abort(errorcode, &
               'Out of memory when initialising 2DECOMP')
       end if
    end if
...
#else
    call mem_split_real(1,src,s1,s2,s3,work1_r,dims(1),decomp%x1dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_r,decomp%x1count,real_type, &
         work2_r,decomp%y1count,real_type, MPI_COMM_COL, ierror)
#else
    call MPI_ALLTOALLV(work1_r,decomp%x1cnts,decomp%x1disp,real_type, &
         work2_r,decomp%y1cnts,decomp%y1disp,real_type, MPI_COMM_COL, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_real(1,work2_r,d1,d2,d3,dst,dims(1),decomp%y1dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_x_to_y_real_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_real(1,src,s1,s2,s3,sbuf,dims(1),decomp%x1dist,decomp)
    
#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%x1count,real_type, &
         rbuf,decomp%y1count,real_type, MPI_COMM_COL, handle, ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%x1cnts,decomp%x1disp,real_type, &
         rbuf,decomp%y1cnts,decomp%y1disp,real_type, MPI_COMM_COL, &
         handle, ierror)
#endif
  ...
  subroutine transpose_x_to_y_real_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_real(1,rbuf,d1,d2,d3,dst,dims(1),decomp%y1dist,decomp)
...
#else
    call mem_split_complex(1,src,s1,s2,s3,work1_c,dims(1),decomp%x1dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_c,decomp%x1count,complex_type, &
         work2_c,decomp%y1count,complex_type, MPI_COMM_COL, ierror)
#else
    call MPI_ALLTOALLV(work1_c,decomp%x1cnts,decomp%x1disp,complex_type, &
         work2_c,decomp%y1cnts,decomp%y1disp,complex_type, MPI_COMM_COL, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_complex(1,work2_c,d1,d2,d3,dst,dims(1),decomp%y1dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_x_to_y_complex_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_complex(1,src,s1,s2,s3,sbuf,dims(1),decomp%x1dist,decomp)
    
#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%x1count,complex_type, &
         rbuf,decomp%y1count,complex_type, MPI_COMM_COL, handle, ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%x1cnts,decomp%x1disp,complex_type, &
         rbuf,decomp%y1cnts,decomp%y1disp,complex_type, MPI_COMM_COL, &
         handle, ierror)
#endif
  ...
  subroutine transpose_x_to_y_complex_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_complex(1,rbuf,d1,d2,d3,dst,dims(1),decomp%y1dist,decomp)
...
#else
    call mem_split_real(2,src,s1,s2,s3,work1_r,dims(2),decomp%y2dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_r,decomp%y2count,real_type, &
...
#ifdef OCC
  subroutine transpose_y_to_z_real_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_real(2,src,s1,s2,s3,sbuf,dims(2),decomp%y2dist,decomp)

#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%y2count,real_type, &
         rbuf,decomp%z2count,real_type, MPI_COMM_ROW, handle, ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%y2cnts,decomp%y2disp,real_type, &
         rbuf,decomp%z2cnts,decomp%z2disp,real_type,MPI_COMM_ROW, &
         handle,ierror)
#endif
  ...
  subroutine transpose_y_to_z_real_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
...
#else
    call mem_split_complex(2,src,s1,s2,s3,work1_c,dims(2),decomp%y2dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_c,decomp%y2count,complex_type, &
         dst,decomp%z2count,complex_type, MPI_COMM_ROW, ierror)
#else
    call MPI_ALLTOALLV(work1_c,decomp%y2cnts,decomp%y2disp,complex_type, &
...
#ifdef OCC
  subroutine transpose_y_to_z_complex_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_complex(2,src,s1,s2,s3,sbuf,dims(2),decomp%y2dist,decomp)

#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%y2count,complex_type, &
         rbuf,decomp%z2count,complex_type, MPI_COMM_ROW, handle, ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%y2cnts,decomp%y2disp,complex_type, &
         rbuf,decomp%z2cnts,decomp%z2disp,complex_type,MPI_COMM_ROW, &
         handle,ierror)
#endif
  ...
  subroutine transpose_y_to_z_complex_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
  ...
  subroutine transpose_z_to_y_real(src,dst,opt_decomp)
    ...
    call MPI_ALLTOALL(  src,decomp%z2count,real_type, &
         work2_r,decomp%y2count,real_type, MPI_COMM_ROW, ierror)
#else
    call MPI_ALLTOALLV(  src,decomp%z2cnts,decomp%z2disp,real_type, &
         work2_r,decomp%y2cnts,decomp%y2disp,real_type, MPI_COMM_ROW, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_real(3,work2_r,d1,d2,d3,dst,dims(2),decomp%y2dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_z_to_y_real_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    call NBC_IALLTOALL(  sbuf,decomp%z2count,real_type, &
         rbuf,decomp%y2count,real_type, MPI_COMM_ROW, handle, ierror)
#else
    call NBC_IALLTOALLV(  sbuf,decomp%z2cnts,decomp%z2disp,real_type, &
         rbuf,decomp%y2cnts,decomp%y2disp,real_type, MPI_COMM_ROW, &
         handle, ierror)
#endif
  ...
  subroutine transpose_z_to_y_real_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_real(3,rbuf,d1,d2,d3,dst,dims(2),decomp%y2dist,decomp)
  ...
  subroutine transpose_z_to_y_complex(src,dst,opt_decomp)
    ...
    call MPI_ALLTOALL(  src,decomp%z2count,complex_type, &
         work2_c,decomp%y2count,complex_type, MPI_COMM_ROW, ierror)
#else
    call MPI_ALLTOALLV(  src,decomp%z2cnts,decomp%z2disp,complex_type, &
         work2_c,decomp%y2cnts,decomp%y2disp,complex_type, MPI_COMM_ROW, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_complex(3,work2_c,d1,d2,d3,dst,dims(2),decomp%y2dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_z_to_y_complex_start(handle,src,dst,sbuf,rbuf,opt_decomp)
...
#ifdef EVEN
    call NBC_IALLTOALL(  sbuf,decomp%z2count,complex_type, &
         rbuf,decomp%y2count,complex_type, MPI_COMM_ROW, handle, ierror)
#else
    call NBC_IALLTOALLV(  sbuf,decomp%z2cnts,decomp%z2disp,complex_type, &
         rbuf,decomp%y2cnts,decomp%y2disp,complex_type, MPI_COMM_ROW, &
         handle, ierror)
#endif
  ...
  subroutine transpose_z_to_y_complex_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_complex(3,rbuf,d1,d2,d3,dst,dims(2),decomp%y2dist,decomp)
...
#else
    call mem_split_real(4,src,s1,s2,s3,work1_r,dims(1),decomp%y1dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_r,decomp%y1count,real_type, &
         work2_r,decomp%x1count,real_type, MPI_COMM_COL, ierror)
#else
    call MPI_ALLTOALLV(work1_r,decomp%y1cnts,decomp%y1disp,real_type, &
         work2_r,decomp%x1cnts,decomp%x1disp,real_type, MPI_COMM_COL, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_real(4,work2_r,d1,d2,d3,dst,dims(1),decomp%x1dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_y_to_x_real_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_real(4,src,s1,s2,s3,sbuf,dims(1),decomp%y1dist,decomp)

#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%y1count,real_type, &
         rbuf,decomp%x1count,real_type, MPI_COMM_COL, handle,ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%y1cnts,decomp%y1disp,real_type, &
         rbuf,decomp%x1cnts,decomp%x1disp,real_type, MPI_COMM_COL, &
         handle, ierror)
#endif
  ...
  subroutine transpose_y_to_x_real_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_real(4,rbuf,d1,d2,d3,dst,dims(1),decomp%x1dist,decomp)
...
#else
    call mem_split_complex(4,src,s1,s2,s3,work1_c,dims(1),decomp%y1dist,decomp)
#endif
...
#else
#ifdef EVEN
    call MPI_ALLTOALL(work1_c,decomp%y1count,complex_type, &
         work2_c,decomp%x1count,complex_type, MPI_COMM_COL, ierror)
#else
    call MPI_ALLTOALLV(work1_c,decomp%y1cnts,decomp%y1disp,complex_type, &
         work2_c,decomp%x1cnts,decomp%x1disp,complex_type, MPI_COMM_COL, &
         ierror)
#endif
#endif
...
#else
    call mem_merge_complex(4,work2_c,d1,d2,d3,dst,dims(1),decomp%x1dist,decomp)
#endif
...
#ifdef OCC
  subroutine transpose_y_to_x_complex_start(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange source array as send buffer
    call mem_split_complex(4,src,s1,s2,s3,sbuf,dims(1),decomp%y1dist,decomp)

#ifdef EVEN
    call NBC_IALLTOALL(sbuf,decomp%y1count,complex_type, &
         rbuf,decomp%x1count,complex_type, MPI_COMM_COL, handle,ierror)
#else
    call NBC_IALLTOALLV(sbuf,decomp%y1cnts,decomp%y1disp,complex_type, &
         rbuf,decomp%x1cnts,decomp%x1disp,complex_type, MPI_COMM_COL, &
         handle, ierror)
#endif
  ...
  subroutine transpose_y_to_x_complex_wait(handle,src,dst,sbuf,rbuf,opt_decomp)
    ...
    ! rearrange receive buffer
    call mem_merge_complex(4,rbuf,d1,d2,d3,dst,dims(1),decomp%x1dist,decomp)
